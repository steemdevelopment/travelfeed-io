...
future problems: * https://github.com/steemit/steemit.com/blob/4d4fe1f7da37d3dbb35bd0a131d9e5b44bad316d/app/utils/Links.js
 */
...
/**
    Unless your using a 'g' (glob) flag you can store and re-use your regular e
    xpression.  Use the cache below.  If your using a glob 
    (for example: replace all), the regex object becomes stateful and continues 
    where it left off when called with the same string so naturally the regexp 
    object can't be cached for long.
*/
...
// About performance
// Using exec on the same regex object requires a new regex to be created and
// compile for each text (ex: post).  Instead replace can be used
// `body.replace(remoteRe, l => {` discarding the result for
// better performance`})
// Re-compiling is a chrome bottleneck but did not effect nodejs.
